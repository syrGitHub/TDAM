...
def build_model(X_train):
    ...
    print(dense.shape)#(?, 96, 3)

    x = keras.layers.pooling.GlobalAveragePooling1D()(dense)
    print(x.shape)#(?, 3)
    x=Softmax()(x)
    print(x.shape)#(?, 3)
    feature=RepeatVector(X_train.shape[1])(x)
    print(feature.shape)#(?, 96, 3)

    L=Multiply()([dense,feature])
    print(L.shape)#(?, 96, 3)

    data = Permute((2,1))(dense)
    print(data.shape)#(?,3,96)
...
if __name__ == '__main__':
    i = 132  # 48 72 96 120 144 168 136
    predict_length = 24
    ...
    # CUDA_VISIBLE_DEVICES="" PYTHONHASHSEED=0 python -u best.py | tee ./132_24
